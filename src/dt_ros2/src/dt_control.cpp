#include "dt_ros2/dt_control.hpp"
...
DtControl::DtControl(void) : Node("dt_ros2_node")
{
  ...
  param_.sub_interval = 0.01;  /* (1 / param_.baudrate) * 10.0f; */
  param_.sub_last_time = rclcpp::Clock().now();

  msg_.battery_info.charge = NAN;
  msg_.battery_info.capacity = NAN;
  msg_.battery_info.design_capacity = NAN;
  msg_.battery_info.power_supply_health = sensor_msgs::msg::BatteryState::POWER_SUPPLY_HEALTH_UNKNOWN;
  msg_.battery_info.power_supply_technology = sensor_msgs::msg::BatteryState::POWER_SUPPLY_TECHNOLOGY_UNKNOWN;
  ...
  pub_.bms              = create_publisher<Bms>                           ("/dt/bms_info", 100);
  ...
  pub_.battery_state    = create_publisher<sensor_msgs::msg::BatteryState>("/dt/battery_state", 100);
...
void DtControl::pkgProcess(void)
{
  ...
  switch((ReceiveCmd)rx_buffer.at(3))
  {
    ...
    case ReceiveCmd::battery_info:
    {
      msg_.frame.cmd_85_battery_info.clear();
      for(int32_t loop = 0; loop < rx_buffer.at(2); loop++)
      {msg_.frame.cmd_85_battery_info.push_back(rx_buffer.at(loop));}
      pub_.frame->publish(msg_.frame);

      uint8_t state        = (uint8_t)rx_buffer.at(4);
      uint8_t charge_state = (uint8_t)rx_buffer.at(5);
      uint16_t voltage     = (uint16_t)((rx_buffer.at(7) << 8) | (rx_buffer.at(6) << 0));
      int16_t current      = (int16_t)((rx_buffer.at(9) << 8) | (rx_buffer.at(8) << 0));
      uint16_t soc         = (uint16_t)((rx_buffer.at(11) << 8) | (rx_buffer.at(10) << 0));
      uint16_t capacity    = (uint16_t)((rx_buffer.at(13) << 8) | (rx_buffer.at(12) << 0));
      int8_t max_temp      = (int8_t)rx_buffer.at(14);
      int8_t min_temp      = (int8_t)rx_buffer.at(15);

      msg_.bms.is_offline                = (state >> 0) & 0x01;
      msg_.bms.is_warning                = (state >> 1) & 0x01;
      msg_.bms.charge_mos_is_open        = (state >> 2) & 0x01;
      msg_.bms.discharge_mos_is_open     = (state >> 3) & 0x01;
      msg_.bms.state                     = charge_state;
      msg_.bms.voltage                   = ((float)voltage) * 0.1f;
      msg_.bms.current                   = ((float)current) * 0.1f;
      msg_.bms.soc                       = ((float)soc) * 0.1f;
      msg_.bms.remain_capacity           = ((float)capacity) * 0.01f;
      msg_.bms.maximum_temperature_value = max_temp;
      msg_.bms.minimum_temperature_value = min_temp;

      msg_.battery_info.header.stamp = this->now();
      msg_.battery_info.voltage      = msg_.bms.voltage;
      msg_.battery_info.current      = msg_.bms.current;
      msg_.battery_info.percentage   = msg_.bms.soc * 0.01;
      msg_.battery_info.temperature  = msg_.bms.maximum_temperature_value;
      msg_.battery_info.present      = !msg_.bms.is_offline;

      if(msg_.bms.state == 0) {msg_.battery_info.power_supply_status = sensor_msgs::msg::BatteryState::POWER_SUPPLY_STATUS_NOT_CHARGING;}
      else if(msg_.bms.state == 1) {msg_.battery_info.power_supply_status = sensor_msgs::msg::BatteryState::POWER_SUPPLY_STATUS_CHARGING;}
      else if(msg_.bms.state == 2) {msg_.battery_info.power_supply_status = sensor_msgs::msg::BatteryState::POWER_SUPPLY_STATUS_DISCHARGING;}
      else {msg_.battery_info.power_supply_status = sensor_msgs::msg::BatteryState::POWER_SUPPLY_STATUS_UNKNOWN;}


      pub_.bms->publish(msg_.bms);
      pub_.battery_state->publish(msg_.battery_info);

      pkg_.battery_upload.setResponse();
      pkg_.get_battery.setResponse();

      pkgInfoDisplay(battery_info, 10, "get battery info success.");

      break;
    }
    case ReceiveCmd::battery_error:
    {
      msg_.frame.cmd_86_battery_error.clear();
      for(int32_t loop = 0; loop < rx_buffer.at(2); loop++)
      {msg_.frame.cmd_86_battery_error.push_back(rx_buffer.at(loop));}
      pub_.frame->publish(msg_.frame);

      msg_.bms.warning.clear();

      for(int32_t warn_loop = 0; warn_loop < 8; warn_loop++)
      {
        msg_.bms.warning.push_back(rx_buffer.at(4 + warn_loop));
      }

      pub_.bms->publish(msg_.bms);

      pkgInfoDisplay(battery_warn, 1, "get battery warning success.");
...
void DtControl::pkgOdomCalculation(double linear, double radian)
{
  ...
  msg_.odometry.header.stamp = rclcpp::Clock().now();
  msg_.odometry.header.frame_id = "odom";
  msg_.odometry.child_frame_id = "base_link";
  ...
  msg_.odometry.pose.pose.position.x = odom_.x;
  msg_.odometry.pose.pose.position.y = odom_.y;
  msg_.odometry.pose.pose.position.z = 0;
  msg_.odometry.pose.pose.orientation = tf2::toMsg(q);
  msg_.odometry.twist.twist.linear.x = vx;
  msg_.odometry.twist.twist.angular.z = vyaw;
  msg_.odometry.pose.covariance = cov_array;
  msg_.odometry.twist.covariance = cov_array;

  pub_.odom->publish(msg_.odometry);
}
...
void DtControl::pkgSetBatteryUpload(bool enable)
{
  std::vector<uint8_t> tx_buffer;
  uint8_t data_buffer[1] = {0};
  ...
  *enable_ptr = enable;
  ...
  pkgFormatFrame(TransmitCmd::battery_upload, 1, data_buffer, tx_buffer);
  serialWrite(tx_buffer);
}
...
void DtControl::pkgGetBattery(void)
{
  std::vector<uint8_t> tx_buffer;
  uint8_t data_buffer[1] = {0};

  pkgFormatFrame(TransmitCmd::get_battery, 1, data_buffer, tx_buffer);
  serialWrite(tx_buffer);
}
...
void DtControl::pkgLoopInit(void)
{
  ...
  pkg_.battery_upload.setOnce(true);
  pkg_.battery_upload.setTimeout(1000);
  pkg_.battery_upload.flagClean();
  ...
  pkg_.get_battery.setOnce(false);
  pkg_.get_battery.setTimeout(1000);
  pkg_.get_battery.flagClean();
...
void DtControl::pkgGetLoop(void)
{
  ...
  for(size_t step_loop = 0; step_loop < pkg_.max_num; step_loop++)
  {
    ...
    {
      code = pkgGetCheck(pkg_.battery_upload, pkg_.step);
      if(code == PkgGetCode::send)    {pkgSetBatteryUpload(true);}
      if(code == PkgGetCode::send)    {logs_info("try open battery upload."); break;}
      if(code == PkgGetCode::get)     {logs_info("open battery upload success."); break;}
      if(code == PkgGetCode::timeout) {logs_error("open battery upload timeout."); break;}
    }

    /* loop get */

    else if(pkg_.step == 10)
    {
    ...
    {
      code = pkgGetCheck(pkg_.get_battery, pkg_.step);
      if(code == PkgGetCode::send)    {pkgGetBattery();}
      if(code == PkgGetCode::send)    {/* logs_info("try get battery info."); */ break;}
      // if(code == PkgGetCode::get)     {logs_info("get battery info success."); break;}
      if(code == PkgGetCode::timeout) {logs_error("get battery info timeout."); break;}
    }
